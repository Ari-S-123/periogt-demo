...
# Optional aliases:
# MODAL_TOKEN_ID=wk_...
# MODAL_TOKEN_SECRET=ws_...
```

- Use workspace Proxy Auth tokens (`wk...` / `ws...`) for `MODAL_KEY` and `MODAL_SECRET`.
- `ak...` / `as...` account API tokens will fail with `401 modal-http: invalid credentials for proxy authorization`.
